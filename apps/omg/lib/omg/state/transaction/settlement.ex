...
defmodule OMG.State.Transaction.Settlement do
  ...
  defstruct [:inputs, :outputs, metadata: @default_metadata, protocol_module: __MODULE__]
  ...
  def new(inputs, outputs, metadata) do
    ...
    tx_data =
      Transaction.Payment.new(inputs, outputs, metadata) |> Map.from_struct() |> Map.put(:protocol_module, __MODULE__)
  ...
  def reconstruct(rlp) do
    ...
    with {:ok, payment_struct} <- Transaction.Payment.reconstruct(rlp) do
      tx_data = payment_struct |> Map.from_struct() |> Map.put(:protocol_module, __MODULE__)
...
defimpl OMG.State.Transaction.Protocol, for: OMG.State.Transaction.Settlement do
  ...
  """
  def get_data_for_rlp(%Transaction.Settlement{inputs: inputs, outputs: outputs, metadata: metadata})
      when Transaction.Settlement.is_metadata(metadata),
      do:
        [
          @settlement_marker,
