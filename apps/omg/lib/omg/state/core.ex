...
defmodule OMG.State.Core do
  ...
  defp non_zero_utxos_from(tx, blknum, tx_index) do
    ...
    |> Enum.with_index()
    |> Enum.filter(fn {output, _index} -> Output.Protocol.can_be_forgotten_from_utxo_set?(output) end)
  ...
  defp deposit_to_utxo(%{blknum: blknum, currency: cur, owner: owner, amount: amount}) do
    {Utxo.position(blknum, 0, 0),
     %Utxo{output: %Output.FungibleMoreVPToken{amount: amount, currency: cur, owner: owner}}}
  end
